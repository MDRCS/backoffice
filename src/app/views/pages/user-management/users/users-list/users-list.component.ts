...
import { CustomerModel, selectAllClients,  } from '../../../../../core/e-commerce';
...
export class UsersListComponent implements OnInit, OnDestroy {
	...
	displayedColumns = ['select', 'id', 'login', 'email', 'fullname', '_roles', 'actions'];
	...
	ngOnInit() {
		...
		// load clients list
		const customersSubscription = this.store.pipe(select(selectAllClients)).subscribe(res => this.allClients = res);
	...
	/* UI */
	/**
	 * Returns user client string
	 *
	 * @param user: User
	 */
	getUserClientStr(user: User): string {
		const titles: string[] = [];
		each(user.client, (clientId: number) => {
			const _client = find(this.allClients, (client: CustomerModel) => client.id === clientId);
			if (_client) {
				titles.push(_client.name);
			}
		});
		return titles.join(', ');
	}
